// cmd/tscli/set/invite/user/cli.go
//
// `tscli set invite user --id <invite-id> --resend`
...
func Command() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if _, err := tscli.Do(
				...
				return fmt.Errorf("failed to resend user invite %s: %w", inviteID, err)
			}
			...
			if len(response) > 0 {
				...
				payload := map[string]string{
					"result": fmt.Sprintf("user invite %s resent", inviteID),
					"action": "resend",
					"id":     inviteID,
				}
	...
	cmd.Flags().BoolVar(&resend, "resend", false, "Resend the user invite")
